...
class CodeforcesApi {
  ...
  Future<Map<String, dynamic>?> fetchUserInfo(String handle) async {
    try {
      final response = await dio.get('user.info', queryParameters: {'handles': handle});
  ...
  Future<int> fetchUserRatingDeltaAvg(String handle) async {
    ...
    try {
      final res = await dio.get('user.rating', queryParameters: {'handle': handle});
      ...
      final ratings = res.data['result'] as List;
        ...
        for (var i = ratings.length - 1; i >= max(0, ratings.length - avgCount); i--) {
